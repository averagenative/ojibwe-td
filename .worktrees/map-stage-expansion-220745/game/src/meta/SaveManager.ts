...
interface SaveData {
  ...
  currency:        number;
  unlocks:         string[];
  /** Last stage the player started or retried. Restored on next session. */
  lastPlayedStage: string | null;
}

function defaultSaveData(): SaveData {
  return { version: SCHEMA_VER, currency: 0, unlocks: [], lastPlayedStage: null };
}
...
export class SaveManager {
  ...
  getLastPlayedStage(): string | null {
    return this.data.lastPlayedStage ?? null;
  }

...
import { getStageDef } from '../data/stageDefs';
import type { StageDef } from '../data/stageDefs';
import { SaveManager } from '../meta/SaveManager';

// waveCount is now read from this.stageDef.waveCount (set per stage)
...
export class GameScene extends Phaser.Scene {
  ...
  init(data?: { commanderId?: string; stageId?: string }): void {
  ...
  preload(): void {
    // Resolve the StageDef to find the map pathFile.
    const stage = getStageDef(this.selectedStageId);
    // Fallback gracefully if stage is unknown (e.g. stale save).
    const pathFile = stage?.pathFile ?? 'map-01';
    this.load.json(pathFile, `data/maps/${pathFile}.json`);
    this.load.json('creep-types', 'data/creep-types.json');
    this.load.json('wave-defs',   'data/waves.json');
  }

  create(): void {
    // Resolve the StageDef — used for waveCount and stage metadata.
    this.stageDef  = getStageDef(this.selectedStageId) ?? {
      id: this.selectedStageId, regionId: 'zaagaiganing', name: 'Unknown',
      description: '', waveCount: 20, pathFile: 'map-01', difficulty: 1,
      towerAffinities: [], unlockCost: 0, unlockId: null,
      creepRoster: [], status: 'ready' as const,
    };

    // Persist the selected stage immediately so retry can restore it.
    SaveManager.getInstance().setLastPlayedStage(this.selectedStageId);

    const pathFile = this.stageDef.pathFile;
    this.mapData   = this.cache.json.get(pathFile) as MapData;
    ...
    this.hud.setWave(0, this.stageDef.waveCount);
    ...
    this.events.on('boss-wave-start', (data: { bossKey: string; bossName: string }) => {
      ...
      this.hud.setWave(this.currentWave, this.stageDef.waveCount);
    });
  ...
  private onWaveComplete(waveNum: number): void {
    ...
    if (waveNum >= this.stageDef.waveCount) {
      this.gameState = 'over';
      this.scene.start('GameOverScene', {
        wavesCompleted: this.stageDef.waveCount,
        totalWaves:     this.stageDef.waveCount,
        won:            true,
        runCurrency:    calculateRunCurrency(this.stageDef.waveCount, this.stageDef.waveCount, true),
  ...
  private triggerGameOver(): void {
    ...
    this.scene.start('GameOverScene', {
      ...
      totalWaves:     this.stageDef.waveCount,
      won:            false,
      runCurrency:    calculateRunCurrency(this.currentWave, this.stageDef.waveCount, false),
      stageId:         this.selectedStageId,
      commanderId:    this.selectedCommanderId,
    });
  }

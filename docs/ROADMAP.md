...
### TASK-151 Review Findings (Synergy Offer Targeting Validation)

- **`TOWER_TARGET_DOMAIN` duplicates `targetDomain` from towerDefs.ts** — `OfferManager.ts` hardcodes a `TOWER_TARGET_DOMAIN` map that mirrors the `targetDomain` field already present on each `TowerDef`. If a tower's targeting domain changes in `towerDefs.ts`, the OfferManager copy must be updated manually. Consider deriving `TOWER_TARGET_DOMAIN` from `ALL_TOWER_DEFS` (after adding cannon/mortar to that array) to eliminate the duplication.
- **`ALL_TOWER_DEFS` omits cannon and mortar** — The exported array in `towerDefs.ts` only includes the 6 base towers (arrow, rock-hurler, frost, poison, tesla, aura). Cannon and mortar are exported as individual constants but not in the collection. Adding them to `ALL_TOWER_DEFS` would enable derived maps and simplify iteration across all tower types.
- **Fallback pool in `drawOffers` does not apply `synergyCompatible` filter** — When the eligible pool is exhausted and the draw falls back to already-active offers, incompatible synergy offers could theoretically appear if they were restored from an old save via `restoreFromIds()`. Extremely unlikely in practice (requires all 72+ offers active), but worth noting for defensive hardening.
- **Three synergy offers permanently unreachable** — `voltaic-slime`, `conductor`, and `voltaic-venom` are kept in `offerDefs.ts` but can never be drawn because their tower combos (tesla+poison) are air+ground incompatible. Consider either removing them from the data or adding a `disabled` flag to make the intent explicit, so future developers don't wonder why these offers never appear.

### TASK-139 Review Findings (Give Up / Quit Button)
...
### TASK-029 Review Findings (Map Environment Artifacts)

- **DALL-E 3 sprites not generated** — The acceptance criteria call for 6 DALL-E 3 generated PNGs in `public/assets/environment/`. The current implementation uses Phaser `Graphics` primitives to render environment tiles procedurally, which avoids asset dependencies and file size but does not match the spec. When art polish is prioritised, generate the sprites and swap the `draw*Tile()` calls with `Phaser.GameObjects.Image` objects (preloaded in BootScene).

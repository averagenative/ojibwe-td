...
### TASK-056 — Commander Portrait Idle Animations — Breathing, Expressions, Life (2026-03-02)

Before this task, commander portraits on the selection screen were static images — photographs pinned to a board. There was no sense that these were living companions the player was choosing between. This task brought all six commanders to life through layered procedural animation driven entirely by Phaser tweens and per-frame sine math, with no new sprites required.

**`commanderAnimDefs.ts` — a Phaser-free data layer.** A new module mirrors the pattern established by `creepAnimDefs.ts` and `towerAnimDefs.ts`: it defines the `CommanderAnimDef` interface (breathRateMs, breathAmpY, breathAmpX, personality, element, expressionPool) and per-commander entries for all six commanders. Breathing rates range from 2400 ms (Makoons — powerful, deep) to 3400 ms (Waabizii — serene). Three personality archetypes — wise, aggressive, trickster — each carry a weighted expression pool: wise commanders favour slow blinks and gentle smiles, aggressive commanders lean into brow furrows and hard glances, trickster commanders weight smirks and side glances. The `pickExpression()` helper uses weighted random selection over the pool. Constants `EXPRESSION_MIN_INTERVAL` (5 000 ms) and `EXPRESSION_MAX_INTERVAL` (10 000 ms) bound the micro-animation cadence. The module exports `DEFAULT_COMMANDER_ANIM` for unknown IDs.

**`CommanderSelectScene.ts` — four animation layers.** The scene gains per-card state objects (`CardAnimState`) that track: `breathPhase` (accumulates each frame), `exprTimer` (countdown to next expression), `hovered`, `flashUntil` (suppresses breathing during the selection power-up flash), and a `particles` array. Four new methods drive the animation each frame from `update(time, delta)`: `_stepBreathing()` reads `breathAmpY/X` from the def and applies inverse scaleX/scaleY via `Math.sin(breathPhase)`, with a `rateMultiplier` that doubles the phase advance rate when hovered — making an alert commander breathe noticeably faster under the cursor. `_stepExpressions()` counts down `exprTimer` and fires one of four expression tweens via a switch on the picked type: `blink` pulses scaleY to 0.92 and back in 100 ms; `smirk` tweens the portrait X by +2 px and back in 200 ms; `brow-furrow` nudges Y down 1 px over 300 ms; `glance` drifts X by ±2 px over 400 ms. `_stepAmbientParticles()` spawns element-coloured Graphics circles at portrait edges (green for nature, cyan for ice, yellow for lightning, orange-red for fire, violet for spirit) at 800 ms intervals normally, accelerating to 400 ms when hovered; each particle tweens its alpha from 0.6 to 0 as it drifts upward, then is destroyed. `_stepBorderGlow()` oscillates the border rectangle alpha on a slow sine to give each portrait a breathing outer light.

**Selection feedback.** `highlightCard()` adds a power-up moment: the selected portrait tweens to 1.05× scale then eases back to a confident 1.02×; `flashUntil` is set to `now + 600 ms` so breathing doesn't fight the flash tween. Unselected commanders dim to `alpha: 0.7` via a short tween and slow their breath. `shutdown()` iterates all card states and destroys every particle, preventing Graphics leaks across scene transitions.

**Test coverage.** `commanderIdleAnims.test.ts` adds 96 tests: per-commander data validation (breathRateMs bounds, amplitude bounds ≤ 1.5%/0.8%, valid element, valid personality, non-empty expression pool, positive weights), fallback default correctness, `pickExpression` weighted randomness and edge cases (Math.random = 0/0.999, equal-weight distribution), expression timing constants, personality weight ordering (aggressive brow-furrow > smirk, wise blink > brow-furrow, trickster smirk > brow-furrow), breathing rate ordering (calm > aggressive), element assignments per commander, and 15 structural checks on `CommanderSelectScene.ts` confirming all four animation methods, all four expression case labels, hover/rateMultiplier wiring, hover particle rate constant, selection scale values (1.05/1.02), unselected alpha tween, flashUntil guard, and particle cleanup in shutdown. Total: 1 955 tests passing, 0 type errors.

### TASK-091 — Reorder Arrow Tower to First Slot in Tower Panel (2026-03-02)

Arrow is the cheapest tower at 70 gold and the natural starting purchase for any run — it rewards aim and positioning more than raw economy. Yet it was buried at the end of the tower strip, after Cannon, Frost, Mortar, Poison, Tesla, and Aura. New players would scan left-to-right, find Cannon first, and either overbuy or hesitate. The fix was minimal: a single array reorder in `ALL_TOWER_DEFS` inside `towerDefs.ts` — Arrow moved from position 7 to position 1. Because `TowerPanel` builds its button strip by iterating this array in order and assigns keyboard shortcut hints by index, Arrow now occupies slot 1 with the `1` key, and every other tower shifts right by one position (Cannon: 2, Frost: 3, Mortar: 4, Poison: 5, Tesla: 6, Aura: 7). No TowerPanel logic, no shortcut tables, and no tooltip code required touching — the ordering contract was already respected throughout the codebase.

### TASK-097 — Frost Tower Sound — Less Annoying (2026-03-02)

The frost tower's procedural firing sound was one of the most complained-about pieces of audio in playtesting. Multiple sessions fired the frost tower at high frequencies — a cluster of three frost towers attacking a slow wave could trigger the sound 8–10 times per second — and the original synthesis was a raw descending sine from 3 200 Hz to 2 400 Hz at 0.12 gain, with an instantaneous onset and a hard exponential decay. At that frequency and gain level the sound sat squarely in the most sensitive part of human hearing, and simultaneous triggers stacked into an ear-piercing shimmer. It communicated "ice" but at the cost of making the player want to mute the game.

**The rework** keeps the crystalline character while moving everything away from the harsh register. The base oscillator now starts at 1 100 Hz and descends to 760 Hz — well into a warm mid-tone range — and its peak gain drops from 0.12 to 0.07. More importantly, the onset changes from an instantaneous `setValueAtTime` to a 6 ms linear ramp from zero to peak, which prevents the click-stacking artefact that occurred when many frost towers fired near-simultaneously. A second sine oscillator at twice the frequency (2 200 → 1 520 Hz) is mixed in at only 0.02 peak gain to add a gentle upper harmonic that preserves the crystalline shimmer without the piercing quality. Both oscillators feed through a shared high-pass filter at 140 Hz (Q = 0.7) to strip any sub-bass rumble that the browser's WebAudio engine can introduce on certain hardware. Total duration shortens from 200 ms to 150 ms, which also helps rapid-fire stacking feel less cluttered. The disconnect chain is properly sequenced per oscillator so AudioContext nodes are released as soon as the sound ends.

The change is self-contained to `_sfxFrost()` inside `AudioManager.ts`. No new files, no API surface changes, no test changes required — the existing `AudioManager.test.ts` structural tests continue to confirm the method exists and the `sfxGain` guard path is exercised. Total: 2 022 tests passing, 0 type errors.

## TASK-098: Tower Role Redesign — Rock Hurler Merger & Armor Effectiveness (2026-03-02)

The single biggest balance problem in early playtesting was that Arrow towers dominated every wave. Because all creeps were effectively equal targets and towers only differed in range and rate-of-fire, the cheapest fast-attacking tower (Arrow) was always the optimal choice. Cannon and Mortar existed but offered no meaningful reason to build them over more Arrows.

**The design change** merges Cannon and Mortar into a single "Rock Hurler" tower. Rock Hurler fires a heavy rock that deals direct-hit damage (55 base, 2-second interval) plus AoE splash (45-radius) on impact, making it the natural successor to both cannon and mortar. Crucially, it carries an `armorDamageMult: 1.6` — a 60% damage bonus against armored creeps. Arrow, by contrast, gains `armorDamageMult: 0.3` — a 70% damage penalty against armor. This creates a hard mechanical requirement for tower diversity: armored creep waves cannot be handled by Arrow towers alone.

**Armor mechanics** are wired through the `TowerDef.armorDamageMult` optional field. Values below 1.0 penalise the tower (Arrow vs armor); values above 1.0 grant a bonus (Rock Hurler vs armor). The `isArmored` creep flag — already present on turtle-type creeps — determines whether the multiplier fires. The stage affinity system for the dual-entrance map is updated to recommend `rock-hurler` at convergence zones instead of `mortar`.

**Upgrade tree**: Rock Hurler gets three distinct upgrade paths — Armor Shred (path A: applies % damage vulnerability to hit targets, stacking debuff), Impact Payload (path B: raw damage + execute threshold), and Cluster Submunitions (path C: fires extra fragment projectiles on impact). The cluster path intentionally locks against Armor Shred at tier 3, forcing a specialisation choice.

**Test and data cleanup**: Cannon and Mortar upgrade definitions are removed from `ALL_UPGRADE_DEFS`. A dozen test files updated to reference `rock-hurler` instead of `cannon`/`mortar`. UpgradeManager, BalanceCalc, AudioManager, and inventory tests all updated with correct tier costs and effect values for Rock Hurler. Tower count drops from 7 to 6. Total: 2 023 tests passing, 0 type errors.

## TASK-092: Mobile Audio — Music Not Playing on iOS Chrome (2026-03-02)

Playtesting on iOS (both Safari and Chrome, which share the WebKit engine) revealed that music never played after tapping the PLAY button on the splash screen. The AudioContext would be created but stay suspended, and subsequent calls to `startMusicTrack()` would silently fail because `AudioBufferSourceNode.start()` does nothing on a suspended context.

**The root cause** is iOS WebKit's user-activation policy: `AudioContext.resume()` must be called *synchronously* within a native user-gesture handler (touchstart, click, etc.). The previous implementation called `resume()` from inside `startMusicTrack()`, which is invoked from Phaser's `requestAnimationFrame` loop — not a native gesture stack. iOS silently ignores such calls.

**The fix** introduces a `_pendingTrack` field on `AudioManager`. When `startMusicTrack()` is called with a suspended context, it stores `{ key, fadeMs }` in `_pendingTrack` and calls `ctx.resume()` immediately (synchronously, in case the call stack happens to be on a native gesture). Once the context actually transitions to `'running'` — either via the `.then()` on that call or via the native `touchstart`/`click`/`pointerdown`/`keydown` document listener that was already in place — the new `_onContextResumed()` helper drains the pending track and begins the crossfade. A new `resumeContext()` public method lets call-sites that own a native gesture handler (e.g. the BootScene PLAY button) force a synchronous resume before calling `startMusicTrack()`. `destroy()` clears `_pendingTrack` to prevent a stale queued track from starting after teardown.

Six new tests in `AudioManager.test.ts` cover the full flow: no-start-while-suspended, deferred-start-after-resume-resolves, same-key guard surviving the deferred path, destroy-clears-pending (resume resolves after destroy, track must not start), `resumeContext()` calls `ctx.resume()` when suspended, and `resumeContext()` is a no-op when already running. Total: 2 030 tests passing, 0 type errors.

## TASK-171: Gear Inventory — Style Unification with Meta Upgrades (2026-03-07)

With TASK-170 having introduced a refined dark-panel design language for the Meta Upgrades screen (deep indigo backgrounds, amber accent borders, unified font sizing, and touch-scroll on mobile), the Gear Inventory screen was visually inconsistent by comparison. This task brought InventoryScene into alignment.

**Changes made**: The inventory panel was updated to use the same `0x1a1a2e` background and `0xc8a84b` amber border palette established in TASK-170. Button styles (Back, slot navigation) were standardised to match the rounded-rect shape, font weight, and hover tint used across Meta Upgrades and BetweenWave screens. Touch scrolling was wired into the gear list container using Phaser's input drag listener with pointer-velocity momentum, mirroring the approach from the Meta Upgrades scroll refactor. A 27-test suite (`inventoryStyleUnification.test.ts`) was added to lock in the colour constants, border widths, and scroll behaviour contract so future scene changes can't silently regress the unified look.

## TASK-172: Main Menu — Quick Play Right-Aligned, Start Game Centered (2026-03-07)

The main menu had a layout tension: the Quick Play button sat in an arbitrary position that didn't relate to any anchor, and Start Game was horizontally paired with it rather than centered under the map/stage select area. Both issues made the menu feel unbalanced, especially on mobile.

**Changes made**: Quick Play was repositioned to the right side using a `width * 0.85` relative offset, mirroring the horizontal distance that the Ojibwe TD crest occupies on the left — giving the header visual symmetry. Start Game was pulled into its own horizontal center lane (`width * 0.5`) and placed directly below the region/stage selector so the primary action button flows naturally from the selection UI. All positioning uses relative multipliers rather than hardcoded pixel values, so the layout holds across desktop and mobile resolutions. A 12-test suite (`mainMenuLayoutT172.test.ts`) validates the relative-offset contract for both buttons and confirms the layout remains coherent across a range of common screen sizes.
